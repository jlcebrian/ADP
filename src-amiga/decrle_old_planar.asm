	xdef	DecompressOldRLEToPlanarAsm
	...
	; ULONG	Pixel count
	; WORD 	RLEMask
	; ULONG	Plane stride in bytes

	; Register-remap old-RLE decoder writing plane-major words directly.
...
LOCAL_RLEMASK		equ	ARG_DATA
	...
	; A2 is the only persistent output pointer: plane1/2/3 addresses are derived from it.

DecompressOldRLEToPlanarAsm
...
.nextRun
	...
	; Final partial flush using the single plane0 pointer in A2.
	; The other plane addresses are reconstructed as A2 + N * planeStride.

	move.l	a2,a5
	move.l	ARG_PLANESTRIDE(sp),d3
	move.w	d4,(a2)+
	add.l	d3,a5
	move.w	d5,(a5)
	add.l	d3,a5
...
.skipPlane3
	...
	; Completed one full 16-pixel word from the accumulators: write it to plane-major
	; memory and advance only the plane0 pointer in A2.
